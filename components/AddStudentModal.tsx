...
export default function AddStudentModal({ onClose, onCreated }: Props) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!name.trim() || lessonLimit < 1) return
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error ?? 'Không thể thêm học viên.')
      onCreated(result as StudentSummary)
      onClose()
    } catch (saveError) {
    ...
    >
      ...
      >
        <div className="mb-5 flex items-start justify-between">
          <div>
            <p className="mb-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-[#534ab7] dark:text-[#aaa5ed]">Học viên mới</p>
            <h2 id="add-student-title" className="text-xl font-medium">Thêm vào lớp học</h2>
          </div>
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <label className="block text-xs font-semibold text-[#5f5e5a] dark:text-[#c4c2b9]">
            ...
            <div className="relative mt-2">
              <input
                ...
                className="h-11 w-full rounded-xl border border-black/15 bg-[#fbfbf9] px-3 pr-14 text-sm font-normal outline-none transition focus:border-[#534ab7] focus:ring-2 focus:ring-[#534ab7]/10 dark:border-white/15 dark:bg-[#222220]"
              />
              <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-xs text-[#888780]">VNĐ</span>
            </div>
          </label>
          ...
          <label className="flex cursor-pointer items-center justify-between rounded-xl border border-black/10 px-4 py-3 dark:border-white/10">
            <span>
              ...
              <span className="mt-0.5 block text-xs text-[#888780]">Đánh dấu nếu học viên đã thanh toán đủ.</span>
            </span>
            <input
              type="checkbox"
              checked={paid}
